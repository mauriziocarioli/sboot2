...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "sb-process-server") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name sb-process-server apps/sb-process-server:1.0.0
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app sb-process-server:1.0.0
		call oc expose svc/sb-process-server
	) else (
