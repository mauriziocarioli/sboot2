...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "simple-kjar" "sb-process-server")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name sb-process-server apps/sb-process-server:1.0.0
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app sb-process-server:1.0.0
    oc expose svc/sb-process-server
else

	echo "Launching the application locally..."
	pattern="sb-process-server"
	files=( $pattern )
	#cd ${files[0]}
	cd sb-process-server
